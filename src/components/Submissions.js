...
class Submissions extends Component {
    state = { submissionsCount: 0, web3: null, accounts: null, contract: null, submissions: [], modalOpen: false };
    ...
    render() {
        ...
        _.forEachRight(this.state.submissions, (value, index) => {
            ...
            Submissions.push(
                <div className="submission" key={index} style={{display: "flex", alignItems: "center", flexDirection: "column"}}>
                    <h2> {value[1]} </h2>{" "}
                    ...
                    <p> {value[2]} </p>{" "}
                    <button className="auditFormButton" style={{width: "110px"}} onClick={()=>this.setState({modalOpen:true})}>Audit</button>
        ...
        const AuditForm = () => {
            ...
            return(
                <form style={{display: "flex", flexDirection: "column", width: "70%"}} className="auditForm">
                    <h1>Submit Audit Report</h1>
                    <input type="text" placeholder="Title" className="auditInput"/> 
                    <textarea type="text" placeholder="Description" className="auditInput"/>
                    <input type="input" placeholder="Trust Score" className="auditInput"/>
                    <button type="submit" className="auditFormButton" onClick={()=>alert("Audit report submitted! It is being reviewed")}>Submit</button>
                </form>
            )
        }
        ...
        return (
            <div>
                <h4>A total of {this.state.submissionsCount} news submission(s) currently exist on the blockchain</h4>
                {Submissions}
                <Modal
